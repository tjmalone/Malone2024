function fParam = fParamFRAP(saveOn)
%% fParamFRAP.m
% Contains parameters for autoFRAP experiments. Can create new parameter
% files or can be used for ad hoc parameter generation. Use fParamBase or
% fParamBleach for their respective experiments.
%
...
fParam.acq.powers = [30,1];         % beam powers [green,red]
...
fParam.gen.times = [-1 0:10:60]*60;             % time points
...
fParam.gen.timesLim = 1*60;                     % buffer before time switch 
% convert buffer to post-bleach time
if length(fParam.gen.times)>=3
    fParam.gen.clockLim = fParam.gen.times(3)-fParam.gen.timesLim;
end
fParam.gen.save = 1;        % whether to save stacks (1=save)
...
fParam.gen.offset = -40;    % initial z-axis offset (distance from Ocular)
...
fParam.bl.powers = 70;              % laser power during bleach
fParam.bl.frNum = 10;               % number of frames for bleach
...
% individual parameters can be turned on and off when calling dataCheck

fParam.dat.nrmBase = 2;             % time to normalize to
